...
describe("validateClaim", () => {
  it("passes a grounded claim whose citation exists", () => {
    ...
    expect(
      validateClaim(
        base({ provenance: "grounded", citations: [{ file: "src/a.ts" }] }),
        { repoRoot: root },
      ),
    ).toEqual([]);
  });
  it("flags a grounded claim with no citations", () => {
    ...
    const issues = validateClaim(
      base({ provenance: "grounded", citations: [] }),
      { repoRoot: root },
    );
  ...
  it("flags a grounded citation that does not exist", () => {
    const root = repoWith("src/a.ts");
    const issues = validateClaim(
      base({ provenance: "grounded", citations: [{ file: "src/ghost.ts" }] }),
      { repoRoot: root },
    );
  ...
  it("forbids speculation as the headline of a recommendation", () => {
    const root = repoWith("src/a.ts");
    const issues = validateClaim(base({ provenance: "speculation" }), {
      repoRoot: root,
      isHeadline: true,
    });
  ...
  it("requires a falsification tuple when recorded as a forecast", () => {
    const root = repoWith("src/a.ts");
    const issues = validateClaim(base({ falsification: undefined }), {
      repoRoot: root,
      isForecast: true,
    });
  ...
  it("flags an absolute citation outside the repo", () => {
    const root = repoWith("src/a.ts");
    const issues = validateClaim(
      base({ provenance: "grounded", citations: [{ file: "/etc/hosts" }] }),
      { repoRoot: root },
    );
    expect(issues.map((i) => i.code)).toContain("citation_escapes_repo");
  });
  it("flags a sibling dir that shares the root's name prefix", () => {
    ...
    const issues = validateClaim(
      base({
        provenance: "grounded",
        citations: [{ file: join(sibling, "secret") }],
      }),
      { repoRoot: root },
    );
    expect(issues.map((i) => i.code)).toContain("citation_escapes_repo");
